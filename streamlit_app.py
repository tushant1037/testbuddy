...
def get_bedrock_client():
    try:
        # Try to create client with environment variables or AWS config
        # return boto3.client('bedrock-runtime', region_name='us-east-1', aws_access_key_id='', aws_secret_access_key='')
        return boto3.client(
                    "bedrock-runtime",
        ...
        if os.getenv('AWS_ACCESS_KEY_ID'):
            st.info("✅ AWS_ACCESS_KEY_ID found in environment")
        else:
            st.warning("❌ AWS_ACCESS_KEY_ID not found in environment")
            
        if os.getenv('AWS_SECRET_ACCESS_KEY'):
            st.info("✅ AWS_SECRET_ACCESS_KEY found in environment")
        else:
            st.warning("❌ AWS_SECRET_ACCESS_KEY not found in environment")
